...
describe("BoxOpener", function () {
    before(async function () {
        this.BoxOpener = await ethers.getContractFactory("BoxOpener");
    ...
    beforeEach(async function () {
        ...
        this.boxOpener = await upgrades.deployProxy(
            this.BoxOpener, 
            [this.poker.address, this.pokerBox.address, this.cryptoPoker.address, owner.address],
            { initializer: 'initialize' }
        );
        await this.boxOpener.deployed();
        console.log("BoxOpener deployed to: ", this.boxOpener.address);
    });

    it("Should start newSailPeriod success when the caller is owner", async function () {
        const [owner] = await ethers.getSigners();
        await expect(
            this.boxOpener.newSailPeriod(Math.floor(Date.now() / 1000)+100, 36000, 100, 1, 5)
        ).to.emit(this.boxOpener, "NewPeriodStarted");
    });

    it("Should start newSailPeriod failed when the caller is not owner", async function () {
        const [owner, other] = await ethers.getSigners();
        await expect(
            this.boxOpener.connect(other).newSailPeriod(Math.floor(Date.now() / 1000) + 20, 36000, 100, 1, 5)
        ).to.be.revertedWith("Ownable: caller is not the owner");
    });

    it("Should start newSailPeriod failed when the start timestamp less than current timestamp", async function () {
        const [owner] = await ethers.getSigners();
        await expect(
            this.boxOpener.newSailPeriod(Math.floor(Date.now() / 1000) - 10, 36000, 100, 1, 5)
        ).to.be.revertedWith("newSailPeriod: timestamp parameter error");
    });

    it("Should buyBox success when all parameter meet with the requirements", async function () {
        const [owner] = await ethers.getSigners();
        const keccak256 = require('keccak256');
        await expect(
            this.pokerBox.grantRole('0x' + keccak256('MINTER_ROLE').toString('hex'), this.boxOpener.address)
        ).to.emit(this.pokerBox, 'RoleGranted');

        await this.poker.mint(owner.address, 100);
        await this.poker.approve(this.boxOpener.address, 100);

        await expect(
            this.boxOpener.newSailPeriod(Math.floor(Date.now() / 1000) + 70, 3600, 100, 1, 5)
        ).to.emit(this.boxOpener, "NewPeriodStarted");
        ...
        await this.boxOpener.buyBox(1);

        expect(await this.pokerBox.balanceOf(owner.address)).to.equal(1);
    });

    it("Should buyBox failed when not on the sail period", async function () {
        const [owner] = await ethers.getSigners();
        await this.poker.mint(owner.address, 100);
        await this.poker.approve(this.boxOpener.address, 100);
        ...
        await expect(
            this.boxOpener.newSailPeriod(Math.floor(Date.now() / 1000) + 100, 3600, 100, 1, 5)
        ).to.emit(this.boxOpener, "NewPeriodStarted");

        await expect(
            this.boxOpener.buyBox(1)
        ).to.revertedWith("buyBox: box not on sail now.");
    });

    it("Should buyBox failed when buy count bigger than limit", async function () {
        const [owner] = await ethers.getSigners();
        await this.poker.mint(owner.address, 100);
        await this.poker.approve(this.boxOpener.address, 100);
        ...
        await expect(
            this.boxOpener.buyBox(1)
        ).to.emit(this.boxOpener, "BoxBought");
        await expect(
            this.boxOpener.buyBox(2)
        ).to.emit(this.boxOpener, "BoxBought");
        await expect(
            this.boxOpener.buyBox(3)
        ).to.revertedWith('buyBox: buy count limit');
    });
    ...
    it("Should openBox success when all parameter meet with the requirements", async function () {
        const [owner] = await ethers.getSigners();
        await this.poker.mint(owner.address, 100);
        await this.poker.approve(this.boxOpener.address, 100);
        ...
        await expect(
            this.cryptoPoker.grantRole('0x' + keccak256('MINTER_ROLE').toString('hex'), this.boxOpener.address)
        ...
        await expect(
            this.boxOpener.buyBox(5)
        ).to.emit(this.boxOpener, "BoxBought");

        expect(await this.pokerBox.balanceOf(owner.address)).to.equal(5);
        expect(await this.cryptoPoker.balanceOf(owner.address)).to.equal(0);

        let boxes = await this.pokerBox.getUserTokensWithPaged(owner.address, 0, 5);
        console.log(boxes);

        await this.pokerBox.setApprovalForAll(this.boxOpener.address, true);

        await expect(
            this.boxOpener.openBox(1)
        ).to.emit(this.boxOpener, "BoxOpened");

        expect(await this.pokerBox.balanceOf(owner.address)).to.equal(4);
        expect(await this.cryptoPoker.balanceOf(owner.address)).to.equal(1);

        await expect(
            this.boxOpener.openMultipleBox([2, 3, 4, 5])
        ).to.emit(this.boxOpener, "BoxOpened");
        expect(await this.pokerBox.balanceOf(owner.address)).to.equal(0);
        expect(await this.cryptoPoker.balanceOf(owner.address)).to.equal(5);

        pokers = await this.cryptoPoker.getUserTokensWithPaged(owner.address, 0, 5);
        console.log(pokers);
    });
})
